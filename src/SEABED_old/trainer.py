...
class Trainer(object):
    ...
    def fit(self):
        ...
        with open(self.result_dir / f"pathlength_SEABED_{self.args.dataset}_{self.args.unsupervised_approach}.json", "w", encoding="utf-8") as handle:
    ...
    def save(self, epoch):
        torch.save(
            ...
            self.model_dir / f"{self.args.dataset}_{epoch}_{self.args.model_name}_{self.args.unsupervised_approach}.pt",
        )
    ...
    def cal_pk(num, pre, gt):
        pairs = list(zip(gt, pre))
        pairs.sort()
        ranked = [(item[1], item[0], index) for index, item in enumerate(pairs)]
        ranked.sort()
        return sum(1 for index in range(min(num, len(ranked))) if ranked[index][2] < num) / num
    ...
    def score(self, testing_graph_set="test", test_k=100, top_k_approach="parallel"):
        ...
        rho = round(np.nanmean(rho), 3)
        tau = round(np.nanmean(tau), 3)
        pk10 = round(np.mean(pk10), 3)
        pk20 = round(np.mean(pk20), 3)

        self.results.append(("model_name", "topk_approach", "dataset", "graph_set", "#testing_pairs", "time_usage(s/p)", "mae", "acc", "fea", "rho", "tau", "pk10", "pk20"))
        self.results.append((self.args.model_name, top_k_approach, self.args.dataset, testing_graph_set, num, time_usage, mae, acc, fea, rho, tau, pk10, pk20))
        print(*self.results[-2], sep="\t")
        print(*self.results[-1], sep="\t")
