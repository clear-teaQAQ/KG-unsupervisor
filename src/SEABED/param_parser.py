...
def parameter_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="1-based GED column in *_GEDINFO.json pairs_info. Use 3 for the SEABED main GED; use 4 only for LUBM/SWDF no-edge-label ablations.",
    )
    parser.add_argument(
        "--cost-mode",
        choices=["unit", "containment"],
        default="unit",
        help="How to convert a predicted matching into GED. 'unit' keeps the current edge-mismatch style cost; 'containment' fits expansion-style pairs such as YAGO.",
    )
    parser.add_argument(
        "--topk-approach",
    ...
    parser.add_argument("--max-train-pairs", type=int, default=0, help="Limit training pairs for smoke/debug runs. 0 uses all pairs.")
    parser.add_argument("--max-val-pairs", type=int, default=0, help="Limit validation pairs for smoke/debug runs. 0 uses all pairs.")
    parser.add_argument("--max-test-pairs", type=int, default=0, help="Limit testing pairs for smoke/debug runs. 0 uses all pairs.")
    ...
    parser.add_argument("--model-name", type=str, default="GEDRankerSEABED", help="Model name.")
