...
def load_pair_file(file_path, name_to_gid, ged_column=3):
    ...
    for entry in payload["pairs_info"]:
        if ged_index >= len(entry):
            raise ValueError(f"{file_path} pair has {len(entry)} columns; cannot read --ged-column {ged_column}: {entry}")
...
def load_dataset(dataset_root, use_raw_features=True, ged_column=3):
    ...
    for split in ["train", "val", "test"]:
        ...
        pairs[split] = load_pair_file(pair_file, global_name_to_gid, ged_column=ged_column)
