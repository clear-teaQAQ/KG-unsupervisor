...
from layers import AGNN, timestep_embedding, ScalarEmbeddingSine, AGNN_D
...
class Discriminator(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        for l in range(self.num_layers):
            if l == 0:
                ...
                agnn = AGNN_D(self.hidden_dims[l],self.hidden_dims[l])
            else:
                nn = torch.nn.Sequential(
                ...
                agnn = AGNN_D(self.hidden_dims[l],self.hidden_dims[l-1])
        ...
        self.costMatrix = torch.nn.Sequential(torch.nn.Linear(self.hidden_dims[-1],self.hidden_dims[-1]*2),
                            torch.nn.ReLU(),
                            torch.nn.Linear(self.hidden_dims[-1]*2,self.hidden_dims[-1]),
                            torch.nn.ReLU(),
                            torch.nn.Linear(self.hidden_dims[-1],1))
    
    def convolutional_pass(self, features,graph_edge_index,edge_mapping_idx,noise_mapping_emb,batch,graph_2):
        bn_batch = batch * 2
        bn_batch[graph_2] += 1
        
        for l in range(self.num_layers):
            features = torch.nn.functional.relu(self.gns[l](self.conv_layers[l](features,graph_edge_index),batch=bn_batch))
            features, noise_mapping_emb = self.agnn_layers[l](features,edge_mapping_idx,noise_mapping_emb,batch)
    ...
    def forward(self, data,noise_mapping_attr):
        ...
        abstract_features,noise_mapping_emb = self.convolutional_pass(graph_x,graph_edge_index,undirected_edge_mapping_idx,undirected_noise_mapping_emb,batch,graph_2)
        cost_matrix = self.costMatrix(noise_mapping_emb)
        
        # sum the values for node matching matrix M, and transpose(M)
        _,cost_matrix = to_undirected(undirected_edge_mapping_idx,cost_matrix)
        
        cost_matrix = cost_matrix[(pair_indicator[undirected_edge_mapping_idx[0]]==0).squeeze(1)]  
        score = global_add_pool(cost_matrix,data.batch[edge_mapping_idx[0]])
