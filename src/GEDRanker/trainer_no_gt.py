...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        self.training_data_loader = DataLoader(Dataset_with_idx(self.training_graphs_large),batch_size=self.args.batch_size,shuffle=True)  
    ...
    def init_roll_out(self):
        ...
        for batch,indices in training_data_loader:
            ...
            for index in range(len(indices)):
                ...
                self.training_graphs_large[indices[index]].best_ged = pred_ged[index] 
                self.training_graphs_large[indices[index]].best_mapping_label = pred_solution[batch.batch[batch.edge_index_mapping[0]]==index].to(self.training_graphs_large[indices[index]].best_mapping_label.device)
                self.training_graphs_large[indices[index]].last_mapping_label = pred_solution[batch.batch[batch.edge_index_mapping[0]]==index].to(self.training_graphs_large[indices[index]].best_mapping_label.device)
                self.training_graphs_large[indices[index]].last_ged = pred_ged[index]
    ...
    def fit(self):
        ...
        training_data_loader = self.training_data_loader
        data_len = len(self.training_graphs_large)
        ...
        with tqdm(total= data_len, unit="graph_pairs", leave=True, desc="Epoch",file=sys.stdout) as pbar:
            ...
            for batch,idx in training_data_loader:
                ...
                batch.to(self.device)
                batch_total_loss,batch_D_loss, rollout_ged,curr_best_ged,batch_map_loss,batch_ged_loss,new_solution = self.process_batch(batch,idx)
        ...
        self.results.append(
            ('model_name', 'dataset', 'graph_set', "epoch", "training_time", "generator training_loss","discriminator training_loss","mapping training_loss","ged training loss","new solutions","pred ged","current best ged"))
        self.results.append(
            (self.args.model_name, self.args.dataset, "train", self.cur_epoch + 1, training_time, training_loss,training_d_loss,training_map_loss,training_ged_loss,total_new_solution,total_pred_ged,total_curr_best_ged))

        print(*self.results[-2], sep='\t')
        print(*self.results[-1], sep='\t')
        with open(self.args.abs_path + self.args.result_path + f'pathlength_nogt_GEDRanker_{self.args.dataset}_{self.args.unsupervised_approach}.json','w') as f:
    ...
    def process_batch(self,batch,indices):
        ...
        for index in range(len(indices)):
            self.training_graphs_large[indices[index]].last_mapping_label = pred_solution[batch.batch[batch.edge_index_mapping[0]]==index].to(self.training_graphs_large[indices[index]].last_mapping_label.device)
            self.training_graphs_large[indices[index]].last_ged = (pred_ged[index]).to(self.training_graphs_large[indices[index]].last_ged.device)
            if pred_ged[index] < best_ged[index]:
                new_solution += 1
                self.training_graphs_large[indices[index]].best_ged = (pred_ged[index]).to(self.training_graphs_large[indices[index]].best_ged.device)
                self.training_graphs_large[indices[index]].best_mapping_label = pred_solution[batch.batch[batch.edge_index_mapping[0]]==index].to(self.training_graphs_large[indices[index]].best_mapping_label.device)
        
            
        return losses.item(),D_loss.item(),pred_ged.sum().item(),best_ged.sum().item(),map_loss.item(),ged_loss.item(),new_solution
    ...
    def save(self, epoch):
        torch.save(self.model.state_dict(),
                   self.args.abs_path + self.args.model_path + self.args.dataset + '_' + str(epoch) + '_' + self.args.model_name + '_' + self.args.unsupervised_approach +'_nogt.pt')

    def load(self, epoch):
        self.model.load_state_dict(
            torch.load(self.args.abs_path + self.args.model_path + self.args.dataset + '_' + str(epoch) + '_' + self.args.model_name + '_' + self.args.unsupervised_approach +'_nogt.pt'))
    ...
    def score(self,testing_graph_set='test', test_k=100, top_k_approach='parallel'):
        ...
        self.results.append(('model_name', 'topk_approach' 'dataset', 'graph_set', '#testing_pairs', 'time_usage(s/p)', 'ged'
                            ))
        self.results.append((self.args.model_name, top_k_approach, self.args.dataset, testing_graph_set, num, time_usage, ged
                            ))

        print(*self.results[-2], sep='\t')
        print(*self.results[-1], sep='\t')

        with open(self.args.abs_path + self.args.result_path + f'result_GEDRanker_{self.args.dataset}_{testing_graph_set}_{self.args.unsupervised_approach}_nogt.json','w') as f:
