...
class Trainer(object):
    ...
    def fit(self):
        ...
        with open(self.args.abs_path + self.args.result_path + f'pathlength_GEDRanker_{self.args.dataset}_{self.args.unsupervised_approach}.json','w') as f:
    ...
    def save(self, epoch):
        torch.save(self.model.state_dict(),
                   self.args.abs_path + self.args.model_path + self.args.dataset + '_' + str(epoch) + '_' + self.args.model_name + '_' + self.args.unsupervised_approach +'.pt')

    def load(self, epoch):
        self.model.load_state_dict(
            torch.load(self.args.abs_path + self.args.model_path + self.args.dataset + '_' + str(epoch) + '_' + self.args.model_name + '_' + self.args.unsupervised_approach +'.pt'))
    ...
    def score(self,testing_graph_set='test', test_k=100, top_k_approach='parallel'):
        ...
        with open(self.args.abs_path + self.args.result_path + f'result_GEDRanker_{self.args.dataset}_{testing_graph_set}_{self.args.unsupervised_approach}.json','w') as f:
