...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        self.training_data_small_loader = DataLoader(self.training_graphs_small,batch_size=self.args.batch_size,shuffle=True)  
        ...
        self.testing_data_unseen_loader = DataLoader(self.testing_graphs_unseen,batch_size=1,shuffle=False)

    def setup_model(self):
        if self.args.model_name == "GEDHOT" or self.args.model_name == "GEDIOT":
    ...
    def init_graph_pairs(self):
        ...
        self.training_graphs_small = []
        self.val_graphs = []
        self.testing_graphs = []
        self.testing_graphs_small = []
        self.testing_graphs_large = []
        self.testing_graphs_unseen = []
            ...
            if self.gn[i] <= 10:
                for j in range(i, train_num):
                    ...
                    if tmp is not None:
                        ...
                        self.training_graphs_small.append(pair)
        ...
        for i in range(val_num, test_num):
            if self.gn[i] <= 10:
                random.shuffle(li_unseen)
                for j in li_unseen[:self.args.num_testing_graphs]:
                    pair = self.pack_graph_pair((0, i, j))
                    self.testing_graphs_unseen.append(pair)
        ...
        print("Generate {} unseen testing graph pairs.".format(len(self.testing_graphs_unseen)))
        print("Generation time:",end-start)
    
    def fit(self):
        if self.args.trainingset == 'training':
            training_data_loader = self.training_data_loader
            data_len = len(self.training_graphs)
        elif self.args.trainingset == 'small':
            training_data_loader = self.training_data_small_loader
            data_len = len(self.training_graphs_small)
        print("\nModel training.\n")
        t1 = time.time()
    ...
    def process_batch(self,batch):
        ...
        prediction, pred_ged, pred_mapping_label = self.model(batch)
        losses = (10.0-weight)*mapping_loss(pred_mapping_label,batch) + weight * F.mse_loss(target.unsqueeze(-1),prediction,reduction='sum')
    ...
    def score(self,testing_graph_set='test', test_k=100):
        ...
        elif testing_graph_set == 'unseen':
            loader = self.testing_data_unseen_loader
