...
class Trainer(object):
    ...
    def _pair_adjacency(self, batch, batch_idx):
        ...
        adj_1 = torch.zeros((n1, n1), dtype=torch.float, device=batch.edge_index.device)
        adj_2 = torch.zeros((n2, n2), dtype=torch.float, device=batch.edge_index.device)

        edge_mask_1 = pair_indicator == 0
        if edge_mask_1.any():
            local_edges_1 = pair_edges[:, edge_mask_1] - node_offset
            adj_1[local_edges_1[0], local_edges_1[1]] = 1.0

        edge_mask_2 = pair_indicator == 1
        if edge_mask_2.any():
            local_edges_2 = pair_edges[:, edge_mask_2] - (node_offset + n1)
            adj_2[local_edges_2[0], local_edges_2[1]] = 1.0
    ...
    def _compute_batch_ged(self, solution_sparse, batch):
        ...
        for batch_idx in range(batch_size):
            ...
            adj_1, adj_2 = self._pair_adjacency(batch, batch_idx)
            padded_adj_1 = torch.zeros((n2, n2), dtype=torch.float, device=batch.edge_index.device)
    ...
    def _compute_single_ged_from_dense_solution(self, solution, data):
        ...
        edge_mask_1 = data.x_indicator[data.edge_index[0]].squeeze(1) == 0
        edge_mask_2 = data.x_indicator[data.edge_index[0]].squeeze(1) == 1
        edge_index_1 = data.edge_index[:, edge_mask_1]
        edge_index_2 = data.edge_index[:, edge_mask_2] - n1

        adj_1 = torch.zeros((n1, n1), dtype=torch.float, device=solution.device)
        adj_2 = torch.zeros((n2, n2), dtype=torch.float, device=solution.device)
        if edge_index_1.numel() > 0:
            adj_1[edge_index_1[0], edge_index_1[1]] = 1.0
        if edge_index_2.numel() > 0:
            adj_2[edge_index_2[0], edge_index_2[1]] = 1.0

        padded_adj_1 = torch.zeros((n2, n2), dtype=torch.float, device=solution.device)
        padded_adj_1[:n1, :n1] = adj_1
        permuted_adj_2 = adj_2.index_select(0, permutation).index_select(1, permutation)
    ...
    def init_graph_pairs(self):
        ...
        self.validation_graphs = [self.pack_graph_pair(pair) for pair in self.split_pairs["val"]]
    ...
    def fit(self):
        ...
        with tqdm(total=len(self.training_graphs), unit="graph_pairs", leave=True, desc="Epoch", file=sys.stdout) as progress:
            g_loss_sum = 0
            d_loss_sum = 0
            map_loss_sum = 0
            ged_loss_sum = 0
            ...
            for batch_index, (batch, idx) in enumerate(self.training_data_loader):
                ...
                progress.set_description(
                    "Epoch_{}: g_loss={} d_loss={} map_loss={} ged_loss={} - Batch_{}: generator loss={}, discrimator loss={}, {} new solutions".format(
                        self.cur_epoch + 1,
                        round(1000 * loss, 3),
                        round(1000 * d_loss, 3),
                        round(1000 * map_loss, 3),
                        round(1000 * ged_loss, 3),
                        batch_index,
                        round(1000 * batch_total_loss / len(batch), 3),
                        round(1000 * batch_D_loss / len(batch), 3),
                        new_solution,
                    )
                )
        ...
        with open(
            self._result_file_path(f"pathlength_SEABED_v1_{self.args.dataset}_{self.args.unsupervised_approach}"),
            "w",
            encoding="utf-8",
        ) as handle:
    ...
    def score(self, testing_graph_set="test", test_k=100, top_k_approach="parallel"):
        ...
        with open(
            self._result_file_path(f"result_SEABED_v1_{self.args.dataset}_{testing_graph_set}_{self.args.unsupervised_approach}"),
            "w",
            encoding="utf-8",
        ) as handle:
            json.dump(
