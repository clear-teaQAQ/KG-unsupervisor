"""V4 trainer: train the unchanged GEDRanker on topology-aligned features."""
...
class CorrectedTrainingTrainer(Trainer):
    ...
    def save(self, epoch):
        super().save(epoch)
        self.saved_checkpoint_path = self._model_checkpoint_path(epoch)

    def score(self, testing_graph_set="test", test_k=100, top_k_approach="parallel"):
        super().score(testing_graph_set, test_k, top_k_approach)
        ...
        result.update(
            {
                ...
                "feature_topology_reindex": self.reindex_diagnostics,
                "checkpoint_path": str(
                    getattr(self, "saved_checkpoint_path", "")
                ),
            }
        )
        with open(result_path, "w", encoding="utf-8") as handle:
            json.dump(result, handle, indent=2)
        print("Recorded V4 training result:", result_path)
        return result
