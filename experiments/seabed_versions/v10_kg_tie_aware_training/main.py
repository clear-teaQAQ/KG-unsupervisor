...
def main():
    ...
    if args.use_raw_features != 1 or args.ged_column != 3 or args.cost_mode != "unit":
        raise ValueError(
            "V10 fixes use_raw_features=1, ged_column=3, and cost_mode=unit."
        )
    if args.model_train != 1 or args.model_epoch_start != 0:
        raise ValueError("V10 trains from scratch with model_train=1 and epoch_start=0.")
    ...
    manifest = {
        ...
        "objective_revision": trainer.objective_revision,
    ...
    print("Saved V10 manifest:", manifest_path)
