...
EPOCHS="${EPOCHS:-200}"
RESULT_PATH="${SCRIPT_DIR}/eval_results"

latest_generator() {
  find "${SCRIPT_DIR}/checkpoints" -maxdepth 1 -type f \
    -name "${1}_${EPOCHS}_GEDRankerSEABED_v19_${V19_MODE}_BPR_*.pt" \
...
for DATASET in ${DATASETS}; do
  ...
  if [[ -z "${GENERATOR}" ]]; then
    echo "V19 generator checkpoint not found for ${DATASET}." >&2
    exit 2
  fi
  ...
  if [[ ! -f "${GENERATOR}" || ! -f "${DISCRIMINATOR}" ]]; then
    echo "V19 checkpoint pair not found for ${DATASET}." >&2
    exit 2
  fi
  if [[ "${SMOKE:-0}" == "1" ]]; then
    TEST_K="${TEST_K:-3}"
