...
ANCHOR_MAX_ITERATIONS="${ANCHOR_MAX_ITERATIONS:-20}"
...
for DATASET in ${DATASETS}; do
  ...
  "${PYTHON_BIN}" experiments/seabed_versions/v8_exact_embedding_anchor/main.py \
    --dataset "${DATASET}" \
    --dataset-root "${DATA_ROOT}/${DATASET}" \
    --checkpoint-path "${CHECKPOINT}" \
    --test-k "${TEST_K}" \
    --max-test-pairs "${MAX_TEST_PAIRS}" \
    --anchor-max-iterations "${ANCHOR_MAX_ITERATIONS}" \
