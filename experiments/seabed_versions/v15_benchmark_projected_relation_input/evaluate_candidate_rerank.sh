...
RESULT_PATH="${SCRIPT_DIR}/candidate_rerank_audit_results"
...
if [[ "${SMOKE:-0}" == "1" ]]; then
  TEST_K="${TEST_K:-3}"
  MAX_TEST_PAIRS="${MAX_TEST_PAIRS:-2}"
else
  TEST_K="${TEST_K:-100}"
  MAX_TEST_PAIRS="${MAX_TEST_PAIRS:-0}"
fi

echo "V15 candidate rerank: checkpoint=${CHECKPOINT_PATH} test_k=${TEST_K} max_test_pairs=${MAX_TEST_PAIRS}"
CHECKPOINT_PATH="${CHECKPOINT_PATH}" \
  RELATION_MODE=raw \
  V15_MODE=projected_input \
  SEED="${SEED:-0}" \
  "${PYTHON_BIN}" "${SCRIPT_DIR}/evaluate_candidate_rerank_main.py" \
    --dataset SWDF \
    ...
    --model-name GEDRankerSEABED_v15_candidate_official_rerank \
